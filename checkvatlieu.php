...
<div class="container content">
  ...
  <div class="col-md-12 left">
  	<div class="breadcum">
    	<img src="images/icon-title-tintuc.png" id="logotitle" /> <p><a href="trangchu">TRANG CHỦ </a> <i class="fa fa-angle-double-right"></i> <a href="danhsachgang.php">Check Vật Liệu</a> </p>
    
    </div>
    <div class="clear"></div>
    <div class="col-md-12 noidung gang">
    	<table width="100%" border="1" cellpadding="25" cellspacing="25"  >
        	<thead>
            	<tr>
                	<th>ID</th>
                    <th>IP</th>
                    ...
                    <th>Mè Đen</th>
                    <th>Gangs</th>
                    <th >Cấp trong Gangs</th>
                </tr>
            </thead>
            <tbody>
            	<?php 
					$qr =  "SELECT * FROM `accounts` order by `Materials` desc";
					$resultgangs = mysqli_query($connect,$qr);
					while($row = mysqli_fetch_array($resultgangs)){
					$online = $row['Online'];
					if($online == 1)
					{
						$kb = '<span style="color:#0F0">Online</span>';
					}
					else
					{
						$kb = "Không Online";
            	...
            	<tr style="height:15px;">
                    ...
                    <td><?php echo adddotstring($row['Coin'])?></td>
                    <td><?php echo adddotstring($row['Materials'])?></td>
                     <td><?php echo adddotstring($row['Pot'])?></td>
                      <td><?php echo adddotstring($row['Crack'])?></td>
                      <td><?php echo adddotstring($row['Hyperion'])?></td>
                      <td><?php echo adddotstring($row['MeDen'])?></td>
                      <td><?php echo adddotstring($row['FMember'])?></td>
                      <td><?php echo adddotstring($row['Rank'])?></td>
